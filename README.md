...
| `observerChunkMaxTokens`    | derived       | Max estimated tokens serialized into one observer chunk. Unset: `floor(contextWindow * 0.2)` of the resolved memory model, or `60000` when the window is unknown. Larger backlogs are drained oldest-first across successive runs. |
