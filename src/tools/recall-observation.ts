...
type ReflectionDetails = Pick<Reflection, "id" | "content" | "supportingObservationIds"> & {
	...
	status: RecalledReflection["status"];
	directlySupersedesReflectionIds: string[];
	directlySupersededByReflectionIds: string[];
...
function reflectionDetails(match: RecalledReflection): ReflectionDetails {
	return {
		...
		status: match.status,
		directlySupersedesReflectionIds: match.directlySupersedesReflectionIds,
		directlySupersededByReflectionIds: match.directlySupersededByReflectionIds,
...
function reflectionLineText(reflection: ReflectionDetails): string {
	...
	if (reflection.directlySupersedesReflectionIds.length > 0) lineage.push(`directly supersedes: ${reflection.directlySupersedesReflectionIds.join(", ")}`);
	if (reflection.supersedesReflectionIds.length > reflection.directlySupersedesReflectionIds.length) lineage.push(`supersedes through lineage: ${reflection.supersedesReflectionIds.join(", ")}`);
	if (reflection.directlySupersededByReflectionIds.length > 0) lineage.push(`directly superseded by: ${reflection.directlySupersededByReflectionIds.join(", ")}`);
	if (reflection.supersededByReflectionIds.length > reflection.directlySupersededByReflectionIds.length) lineage.push(`superseded by through lineage: ${reflection.supersededByReflectionIds.join(", ")}`);
	return `[${reflection.id}]${status} ${reflection.content}${lineage.length > 0 ? ` (${lineage.join("; ")})` : ""}`;
}
...
function reflectionLine(reflection: ReflectionDetails): string {
	...
	if (reflection.status === "superseded") metadata.push("superseded");
	if (reflection.directlySupersedesReflectionIds.length > 0) metadata.push(`supersedes ${reflection.directlySupersedesReflectionIds.join(", ")}`);
	if (reflection.supersedesReflectionIds.length > reflection.directlySupersedesReflectionIds.length) metadata.push(`lineage ${reflection.supersedesReflectionIds.join(", ")}`);
	if (reflection.directlySupersededByReflectionIds.length > 0) metadata.push(`superseded by ${reflection.directlySupersededByReflectionIds.join(", ")}`);
	if (reflection.supersededByReflectionIds.length > reflection.directlySupersededByReflectionIds.length) metadata.push(`lineage from ${reflection.supersededByReflectionIds.join(", ")}`);
	return alignedRow("✓ reflection", metadata.join("; "), reflection.content);
}
...
function noteRows(details: RecallObservationToolDetails, sources: RecallSourceEntryDetails[]): string[] {
	...
	if (details.reflections.some((reflection) => reflection.status === "superseded")) notes.push(noteLine("superseded", "one or more reflections are superseded but remain recallable"));
